...
class Cvl_Manufacturer_Adminhtml_ManufacturerController extends Mage_Adminhtml_Controller_action
{
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Manufacturer'), Mage::helper('adminhtml')->__('Manufacturer'));
	...
	public function saveAction() {
        ...
        $collection=Mage::getModel('manufacturer/manufacturer')->getCollection()->addFieldToFilter('name',$data['name']);
        $manufacturer_data=$collection->getData();
        if(count($collection)>0 && $manufacturer_data[0]['status']==1 && $this->getRequest()->getParam('id')!=$manufacturer_data[0]['id']){
            $this->_forward('edit');
		...
		if ($data) {
			if(isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
				try {	
                  ...
                  //If the uploaded file is not image it will not allow to save manufacturer
